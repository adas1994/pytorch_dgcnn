...
class FrameImageModel(nn.Module):
    def __init__(self, args, output_channels=12):
        ...
        self.conv_block4 = nn.Sequential(nn.Conv2d(96*2, 192, kernel_size=1, bias=False),
                                   self.bn4,
                                   nn.LeakyReLU(negative_slope=0.2))
        self.conv_block5 = nn.Sequential(nn.Conv1d(386, args.emb_dims, kernel_size=1, bias=False),
                                   self.bn5,
                                   nn.LeakyReLU(negative_slope=0.2))
...
class JetClassifier(nn.Module):
    def __init__(self, args, output_channels=5):
        ...
        self.NN1 = JetVarsProcessor(self.args, 64, 12)
        ...
        self.linear3 = nn.Linear(76, 38)
        self.relu3   = nn.LeakyReLU(negative_slope=0.2)
        self.bn3     = nn.BatchNorm1d(38)
        self.dp3     = nn.Dropout(p=args.dropout)

        self.linear4 = nn.Linear(38, 16)
        self.relu4   = nn.LeakyReLU(negative_slope=0.2)
        self.bn4     = nn.BatchNorm1d(16)

        self.linear5 = nn.Linear(16, 5)
    ...
    def forward(self, x, x2):
        ...
        x2        = self.NN1(x2)
        combined1 = torch.cat((combined1, x2),dim=1)
        combined1 = self.bn3(self.relu3(self.linear3(combined1)))
        combined1 = self.bn4(self.relu4(self.linear4(combined1)))
        
        prediction = F.softmax(self.linear5(combined1))
        return prediction
